...
the shared IM logic: /help, /ask, /watch, /buy, /sell, and bare 6-digit code /
stock-name single-stock analysis.

Transport details (sending/editing) go through ``ChannelGateway``; this module
never talks to a platform API directly. Non-editable channels (feishu/weixin)
...
from stock_codex.domain import decision as decision_lib
...
def edit_message(message_id: int | str, text: str, *, format: str = "markdown",
    ...
    非可编辑通道（feishu/weixin）只在 final=True（最终卡片/错误）时落地，
