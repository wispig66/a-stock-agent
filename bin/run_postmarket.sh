...
Use the stock-postmarket skill in this repository. Review today's A-share premarket plan, persist today's sentiment and theme data, and push tomorrow's plan to the IM gateway. Return only a concise operational summary.
PROMPT

  echo
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] 刷新 stock_basic（给单股查询用）"
