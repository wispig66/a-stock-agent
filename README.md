...
它把「盘前计划、盘中纪律、盘后复盘、周复盘、异动监控、随时问答」拆成一组可审计的 Codex skill：先由确定性脚本拉取行情、题材、消息和持仓数据，形成 fact pack；再由 Codex/LLM 生成交易研究卡片；最后通过飞书 / 企业微信 / 个人微信推送给用户。
...
- 飞书是默认通道（官方 WebSocket 长连，无需公网）。
- 企业微信智能机器人（长连接）和个人微信 iLink（扫码、仅 1v1）为可选通道；企业微信适合主动推送，个人微信定时推送为 best-effort。
...
```mermaid
flowchart LR
    User["用户<br/>飞书 / 企业微信 / 微信"] --> Gateway["IM gateway<br/>channel_listener.py"]
    ...
    Validator --> Notify["notify.push<br/>飞书 / 企业微信 / 微信"]
    Notify --> User
...
sequenceDiagram
    ...
    participant Push as 飞书 / 企业微信 / 微信
...
- 你希望通过飞书 / 企业微信 / 微信随时问单股、题材、事件，但数据和记录仍保存在本机。
...
| IM 网关 | `channel_listener.py` | 统一处理飞书 WebSocket、企业微信长连接、微信 iLink 长轮询入站消息，连接绑定型通道经 outbox 出站。 |
...
| 企业微信智能机器人 | 可选；长连接、无需公网，原生支持主动推送。 |
| 个人微信 iLink | 可选；扫码登录、仅 1v1，定时推送 best-effort。 |
...
三个通道可单独或并存启用（`CHANNELS_ENABLED` 逗号分隔）。飞书为默认；企业微信、个人微信为可选。连接绑定型通道（企业微信 / 微信）的出站经 outbox 由监听进程发送，因此 `start_gateway.sh` 需常驻。
...
企业微信智能机器人走长连接（`wss://openws.work.weixin.qq.com`），无需公网回调，原生支持主动推送，适合定时盘前/盘中/盘后卡片。在企业微信管理后台创建智能机器人，拿到 `BotId` 和 `Secret`：

```dotenv
CHANNELS_ENABLED=feishu,wecom
CHANNELS_NOTIFY=feishu,wecom
WECOM_BOT_ID=
WECOM_SECRET=
WECOM_HOME_CHANNEL=
WECOM_ALLOWED_USERS=
...
注意：主动推送要求该会话此前有用户消息（个人自用先给机器人发一句即可）。出站经 outbox 由 `start_gateway.sh` 持有的连接发送。
...
向导会请求登录二维码、等你扫码确认，并把 `WEIXIN_TOKEN` / `WEIXIN_BASE_URL` 写入 `.env`。启用后把 `weixin` 加入 `CHANNELS_ENABLED`，先在微信里给机器人私聊一条消息建立 context_token。
...
stock_codex/             Python 包
  apps/                  runtime 入口
  channels/              飞书 / 企业微信 / 微信 gateway adapter
