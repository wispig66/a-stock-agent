...
**注意（macOS TCC/FDA）**：launchd 模板 `launchd/disabled/com.user.stocktglistener.plist` 默认保持禁用，`scripts/install_runtime_services.sh` 不会自动注册它。原因：项目在 `~/Desktop` 下时 launchd 拉起的 `uv` 进程会在 `getcwd` 阻塞（Desktop 是受 TCC 保护的目录，背景守护进程拿不到 Full Disk Access），表现为 `uv` 卡死无子进程。如果你把项目搬到 `~/code` 之类无 TCC 限制的目录，可以保留模板在 disabled 路径，并显式执行 `ENABLE_TG_LISTENER_LAUNCHD=1 bash scripts/install_runtime_services.sh`；否则一律用 `start_tg_listener.sh` 在交互式 SSH/终端里拉起（继承终端 TCC 权限）。
...
调度分两层：`Codex automations` 负责短时 LLM jobs，`launchd 运行长时 daemon`。Codex automations 必须安装在实际运行交易 workflow 的 runtime host 上；开发机只通过 SSH 触发远程部署。

| 类型 | 任务 | 时间 | 入口 |
|------|------|------|------|
| Codex automations | L1 盘前 | 工作日 08:30 | `stock-premarket` |
...
短时 LLM jobs 包括 L1 盘前、L2 盘中 4 时点、L4 盘后、周复盘；长时 watcher daemon 包括 `watch_loop`、`anomaly_loop`、`theme_loop`，可选 `tg_listener`。只有两个前提：runtime host 不睡眠（接电源时禁止自动睡眠）+ 网络通。

## 安装

需要：[uv](https://docs.astral.sh/uv/)（Python 包管理器，比 pip 快 10-50 倍）和 Codex。开源部署采用 pull-based 模型：runtime host 自己 clone/pull 仓库并安装依赖；开发机只通过 SSH 触发远程部署，不把本地工作区直接同步过去。

### Runtime host 本地安装

在实际运行交易 workflow 的 runtime host 上：
...
开发机不直接运行交易 workflow，只准备远程部署配置并触发 runtime host 自己 pull/install：

```bash
cp deploy.remote.example.env deploy.remote.env
# 编辑 deploy.remote.env，填 runtime host、仓库目录、分支等
bash scripts/deploy_remote_codex.sh
```

`deploy.remote.env` 只保存 SSH 目标、远端目录、仓库地址、分支等部署参数，不保存 Telegram token。runtime host 上的 `.env` 必须已经存在且填好；首次远程部署如果只生成了 `.env` 模板，需要先 SSH 到 runtime host 填写 `.env`，再重跑部署或 doctor。

部署完成后可在 runtime host 上运行：
