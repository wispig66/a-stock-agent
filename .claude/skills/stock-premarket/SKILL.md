...
- 必须通过 `.claude/skills/stock-premarket/scripts/push.py` 推送。
...
```bash
.venv/bin/python .claude/skills/stock-premarket/scripts/fetch_data.py
```
...
- **解禁过滤（30 天硬约束）**：每只候选生成后**必须**执行：
  ```bash
  .venv/bin/python .claude/skills/stock-premarket/scripts/extras.py --lockup <6位代码>
  ```
...
```bash
.venv/bin/python .claude/skills/stock-premarket/scripts/preflight.py
```
...
2. 运行：
   ```bash
   .venv/bin/python .claude/skills/stock-premarket/scripts/push.py --file data/last_card.md
   ```
...
- LLM 看到重要题材时调 `.venv/bin/python .claude/skills/stock-premarket/scripts/fetch_data.py --concept "电力"` 拉成分股详情
