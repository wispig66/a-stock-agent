...
```bash
~/miniconda3/envs/stock/bin/python .claude/skills/stock-premarket/scripts/fetch_data.py
```

读完整输出。这是 fact pack，**六节**：
1. 涨停结构（含连板个股清单）
2. 热门行业 Top 5
3. 炸板池
...
## Step 2 · 抓今日新闻面（24h 窗口）

依次抓，每个源失败就跳，不阻塞：

1. **WebFetch** `https://www.cls.cn/depth/1003` — 财联社早盘速递
2. **WebFetch** `https://www.cls.cn/telegraph` — 财联社电报头条
3. **WebSearch** 限定 2026 年最近 2 天：
   - "证监会 公告"
   - "央行 货币政策"
   - "工信部 OR 发改委 政策"
4. **WebSearch** fact pack 中 Top 3 行业相关产业链消息
5. **WebSearch** 夜盘海外联动：美股纳指 / 中概 ADR / 原油 / 黄金
...
- **解禁过滤（30 天硬约束）**：每只候选生成后**必须**执行：
  ```bash
  ~/miniconda3/envs/stock/bin/python .claude/skills/stock-premarket/scripts/extras.py --lockup <6位代码>
  ```
...
2. 运行：
   ```bash
   ~/miniconda3/envs/stock/bin/python .claude/skills/stock-premarket/scripts/push.py --file data/last_card.md
   ```
...
6. 数据积累中（sentiment_daily 为空）时，阶段判定**必须**标"积累中"
7. 输出卡片 ≤ 2000 字
8. 任何步骤失败明确写"无数据"，不要编
9. 必须包含「9:50 后未触发放弃」和「双轨止损」两条风险提示
10. **观察池每只候选必须查 30 天内解禁**（调 `extras.py --lockup`），`float_ratio > 5%` 的不进观察池
11. **重点题材必须优先采纳同花顺 reason tag 高频项**（fact pack 第 5.1 节），不能纯凭新闻面臆测
12. **新闻必须标注命中维度**（[[feedback-news-awareness]]）：题材驱动 / 持仓利空 / 新主线候选 / 无关。无标注的消息条目视为输出残次。持仓利空必须单列"开盘特别提示"。
