...
> 区分两种"额外工作"：
> - ❌ **代码层异常立即退出**：ImportError / 字段不存在 / lib 源码报错 → 立即走 4d 错误卡。不要换 import 路径、不要探 DB schema、不要 patch lib 源码
> - ✅ **业务层查无结果继续研究**：sector_pack/event_pack 查无结果（属业务数据缺失） → 按 Step 2.5 降级链继续，**不要立即放弃**
...
2. **代码层异常立即退出**：ImportError / 字段不存在 / lib 源码报错 → 立即走 4d 错误卡；不要换 import 路径、不要探 DB schema、不要 patch lib 源码
3. **业务层查无结果继续研究**：sector/event 查无结果（属业务数据缺失，非代码异常） → 必须走 Step 2.5 降级链（LLM 重映射 → event_pack → 含信息的 4c），不要直接放弃
