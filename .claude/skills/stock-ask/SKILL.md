...
> 任何函数抛异常或返回空 → 立即走 Step 4c/4d 兜底卡，**不要尝试 ImportError 调试、不要换 import 路径、不要探数据库**。
...
**下游异常处理**（任一触发立刻走兜底卡，不要继续折腾）：
- `sector` 分支 `build_sector_pack` 抛 `SectorNotFound` / 返回空 → 走 4c 模糊兜底
- `event`  分支 `build_event_pack` 抛异常 / 返回空 → 走 4c 模糊兜底
- `stock`  分支 stock-query 子调用报错 → 走 4d，error_msg = "无法分析该股票"
- 任一步骤超过 60 秒未出结果 → 立即降级到 4c
...
2. **失败必降级**：任一函数异常或返回空 → 立即走兜底卡（4c/4d），不要尝试 ImportError 调试或换 import 路径或探 DB schema
3. 数据缺失项不要瞎编，写"—"或省略行
4. 仅输出卡片，不要解释思考过程
5. 严格使用上面模板格式（不要 markdown 表格，TG 不渲染）
6. 卡片单条 ≤ 800 字（对齐 stock-query）
