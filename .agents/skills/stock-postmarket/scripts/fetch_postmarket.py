...
PREMARKET_SCRIPTS = PROJECT_ROOT / ".agents/skills/stock-premarket/scripts"
...
def fetch_zd(date: str) -> pd.DataFrame:
    ...
    return _fetch_structured_table(
        ...
        cache_name="zd",
        date=date,
        fetcher=lambda: ak.stock_zt_pool_dtgc_em(date=date),
    )
...
def fetch_qs(date: str) -> pd.DataFrame:
    ...
    return _fetch_structured_table(
        ...
        cache_name="qs",
        date=date,
        fetcher=lambda: ak.stock_zt_pool_strong_em(date=date),
    )
...
def _write_df_cache(date: str, cache_name: str, df: pd.DataFrame) -> None:
    ...
    _cache_path(date, cache_name).write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")


def _read_df_cache(date: str, cache_name: str) -> tuple[pd.DataFrame, str | None]:
    path = _cache_path(date, cache_name)
    if not path.exists():
...
def _fetch_structured_table(
    *,
    ...
    cached, cached_at = _read_df_cache(date, cache_name)
    if not cached.empty:
        log(f"{label} 使用缓存快照 {cached_at}（实时源失败: {last_error}）")
        return cached
...
def _cache_meta(df: pd.DataFrame, prefix: str) -> dict:
    if df.attrs.get("snapshot_source") != "cache":
        return {}
    return {
        f"{prefix}_snapshot_source": "cache",
        f"{prefix}_snapshot_at": str(df.attrs.get("snapshot_at") or ""),
    }


def assert_structural_data_available(*, zt: pd.DataFrame, zd: pd.DataFrame, zb: pd.DataFrame, qs: pd.DataFrame) -> None:
    if not zt.empty or not zd.empty or not zb.empty or not qs.empty:
        return
    raise DataUnavailable(
        "postmarket structural data unavailable: 涨停/跌停/炸板/强势股池全部为空；"
        "拒绝写入 0 数据 fact pack"
    )
...
def build_allowed(*, iso: str, sent: dict, zt: pd.DataFrame, zd: pd.DataFrame,
    ...
    summary.update(_cache_meta(zt, "limit_up"))
    summary.update(_cache_meta(zd, "limit_down"))
    summary.update(_cache_meta(zb, "broken"))
