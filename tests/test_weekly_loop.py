...
def test_skip_when_file_exists(tmp_path, monkeypatch):
    """data/weekly_review/<label>.md 已存在 → 跳过，不调 agent。"""
    ...
    with patch.object(weekly_loop, "_invoke_agent") as mock_invoke:
        rc = weekly_loop.main([])
...
def test_force_overrides_skip(tmp_path, monkeypatch):
    ...
    with patch.object(weekly_loop, "_invoke_agent", return_value=0) as mock_invoke:
